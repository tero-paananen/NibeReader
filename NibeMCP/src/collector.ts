...
export async function runCollector(c: Config) {
  ...
  const status: CollectorStatus = {
    ...
    device: identity(c),
    sample_seconds: c.sampleMs / 1000,
    retention_days: c.retentionDays,
    last_poll: null,
    last_successful_sample: null,
    last_error: null,
  };
  let inFlight: Promise<Reading[]> | undefined;
  function snapshot() {
    if (!inFlight) {
      inFlight = readPump(c).finally(() => {
        inFlight = undefined;
      });
    }
    return inFlight;
    ...
    socket.on('data', async chunk => {
      ...
      try {
        ...
        if (request.method === 'status')
          ...
          const readings = (await snapshot()).filter(r =>
            selected.includes(r.metric_id as (typeof metrics)[number]['id'])
          );
  ...
  async function poll() {
    ...
    try {
      ...
      if (good.length)
        status.last_successful_sample = good[good.length - 1].timestamp;
    ...
    if (!status.stopping) {
      const delay = failures
        ? Math.min(
            300000,
            Math.max(c.sampleMs, 5000 * 2 ** Math.min(failures - 1, 6))
          )
        : c.sampleMs;
      // Never catch up missed polls after sleep with a burst of artificial samples.
      timer = setTimeout(() => {
        polling = poll();
      }, Math.max(1000, delay - (Date.now() - begin)));
    }
  }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(c.socketPath, () => {
      chmodSync(c.socketPath, 0o600);
      resolve();
    });
  });
...
if (process.argv[1]?.endsWith('/collector.js')) {
  runCollector(config()).catch(error => {
    console.error(error);
    process.exit(1);
  });
}
