...
export function config(env: NodeJS.ProcessEnv = process.env): Config {
  const dataDir = resolve(
    env.NIBE_DATA_DIR ??
      join(homedir(), 'Library', 'Application Support', 'NibeMCP')
  );
...
export function requireHost(c: Config) {
  if (!c.host)
    throw new Error(
      'Set NIBE_HOST to the heat pump IP address or hostname before reading or starting collection.'
    );
}
