...
export function openHistory(
  ...
  try {
    ...
    `);
    const stored = db
      .prepare("SELECT value FROM metadata WHERE key='device'")
      .get() as {value: string} | undefined;
    if (stored && c.host && stored.value !== identity(c))
      throw new Error(
        'This data directory belongs to a different pump endpoint. Use its original NIBE_HOST or a separate NIBE_DATA_DIR.'
      );
    if (writable && !stored && c.host)
      db.prepare("INSERT INTO metadata VALUES ('device', ?)").run(identity(c));
    return db;
  } catch (error) {
    db.close();
    throw error;
...
export function saveReadings(
  ...
  try {
    ...
    db.prepare('DELETE FROM readings WHERE ts < ?').run(
      now - c.retentionDays * 86400000
    );
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
...
export function historyStatus(c: Config) {
  ...
  try {
    const row = db
      ?.prepare(
        "SELECT MIN(ts) AS first, MAX(ts) AS last, MAX(CASE WHEN quality='ok' THEN ts END) AS success FROM readings WHERE ts >= ?"
      )
      .get(Date.now() - c.retentionDays * 86400000) as
      | {first: number | null; last: number | null; success: number | null}
      | undefined;
    const iso = (v: number | null | undefined) =>
      v == null ? null : new Date(v).toISOString();
    return {
      first_sample: iso(row?.first),
      last_sample: iso(row?.last),
      last_successful_sample: iso(row?.success),
...
export function parseDate(value: string) {
  if (
    ...
    throw new Error(
      'Dates must be ISO 8601 timestamps with an explicit timezone.'
    );
...
export function readHistory(c: Config, request: HistoryRequest) {
  ...
  const start = parseDate(request.start),
    end = parseDate(request.end);
  if (end <= start) throw new Error('end must be later than start');
  if (
    request.interval !== undefined &&
    (!Number.isSafeInteger(request.interval) || request.interval < 1)
  )
  ...
  try {
    ...
    const series = selected.map(metric => {
      const countRow = db
        ?.prepare(
          'SELECT COUNT(*) AS n FROM readings WHERE metric_id=? AND ts>=? AND ts<?'
        )
        .get(metric.id, effectiveStart, end) as {n: number} | undefined;
      const count = countRow?.n ?? 0;
      const width =
        request.interval !== undefined || count > 1000
          ? Math.max(
              (request.interval ?? 1) * 1000,
      ...
      const points =
        !db || !count
          ...
          ? db
              .prepare(
                'SELECT ts, raw_value, value, unit, quality, error FROM readings WHERE metric_id=? AND ts>=? AND ts<? ORDER BY ts'
              )
              .all(metric.id, effectiveStart, end)
              .map(r => ({
                ...r,
          ...
          : db
            ...
            FROM readings WHERE metric_id=? AND ts>=? AND ts<? GROUP BY bucket ORDER BY bucket`
              ...
              .all(start, width, metric.id, effectiveStart, end)
              .map(r => ({
                ...r,
                bucket: undefined,
                timestamp: new Date(
                  start + Number(r.bucket) * width
                ).toISOString(),
                end: new Date(
                  Math.min(end, start + (Number(r.bucket) + 1) * width)
                ).toISOString(),
              }));
      ...
      return {
        metric_id: metric.id,
        unit: metric.unit,
        mode: width === null ? 'raw' : 'aggregate',
        interval_seconds: width === null ? null : width / 1000,
        points,
        gaps: gaps.slice(0, 1000).map(g => ({
          start: new Date(Number(g.s)).toISOString(),
          end: new Date(Number(g.e)).toISOString(),
        })),
    ...
    return {
      ...
      retention_days: c.retentionDays,
      coverage_note:
        'Gaps are intervals without a successful sample covering that time. No interpolation or backfill is performed.',
      series,
    };
  } finally {
    db?.close();
