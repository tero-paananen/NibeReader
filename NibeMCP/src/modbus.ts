...
class ReadSocket extends Socket {
  ...
  override emit(event: string | symbol, ...args: any[]): boolean {
    ...
    while (this.frameBuffer.length >= 7) {
      ...
      catch { this.destroy(new Error('Invalid Modbus response')); return true; }
    }
    return true;
  }
}

function boundedRequest<T>(socket: Socket, timeoutMs: number, action: () => Promise<T>): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => finish(new Error('Modbus request timed out')), timeoutMs);
    const onError = (error: Error) => finish(error);
    const onClose = () => finish(new Error('Modbus connection closed during request'));
    function finish(error?: unknown, value?: T) {
      clearTimeout(timer); socket.off('error', onError); socket.off('close', onClose);
      error ? reject(error) : resolve(value as T);
    }
    socket.once('error', onError);
    socket.once('close', onClose);
    try { action().then(value => finish(undefined, value), error => finish(error)); }
...
export async function readPump(c: Config, ids?: string[]): Promise<Reading[]> {
  ...
  socket.on('error', () => { /* The connect/request promises report errors. */ });
  const failure = (message: string): Reading[] => selected.map(m => ({
    metric_id: m.id, timestamp: new Date().toISOString(), raw_value: null,
    value: null, unit: m.unit, quality: 'error', error: message,
  }));
  try {
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => { socket.destroy(); reject(new Error('Modbus connection timed out')); }, c.timeoutMs);
      const finish = (error?: Error) => { clearTimeout(timer); socket.off('error', onError); error ? reject(error) : resolve(); };
      const onError = (error: Error) => finish(error);
      socket.once('error', onError);
    ...
    for (const m of selected) {
      ...
      try {
        ...
        readings.push({ ...base, timestamp: new Date().toISOString(), raw_value: null, value: null,
          quality: unavailable ? 'unavailable' : 'error', error: e.message ?? String(error) });
        if (e.err !== 'ModbusException') {
          socket.destroy();
          for (const rest of selected.slice(readings.length)) readings.push({
            metric_id: rest.id, timestamp: new Date().toISOString(), unit: rest.unit,
            raw_value: null, value: null, quality: 'error', error: 'Connection failed during this read; retry on the next request.',
          });
          break;
        }
      }
  ...
  finally { socket.destroy(); release(); }
}
